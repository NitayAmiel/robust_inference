...
class PathCurvatureOptimizer_l2:
    def __init__(self, e_hat_burn: np.ndarray, pi: np.ndarray, nb: float, n: int, constraint_sum: float):
        """
    ...
    def objective_function(self, r: float, gamma: float) -> float:
        ...
        # term 1: Empirical variance based on burn-in errors
        empirical_variance = np.sum(self.e_hat_burn / denominator[self.nonzero_mask])
        
        # term 2: Robust optimization penalty (L2 norm of misspecification) 
        robust_penalty = self.constraint_sum * np.sqrt(np.sum(1 / (denominator[self.nonzero_mask] ** 2)))
        
        return (empirical_variance + robust_penalty) / self.n

    def optimize(self, r_steps: int = 50, gamma_steps: int = 11) -> Tuple[float, float, float]:
        """
        Performs 2D grid search to find optimal (rho, gamma) and stores results.
        ...
        best_r = 1.0     # Default to uniform (robust safety)
        best_gamma = 0.0 # Default to geometric
...
class MinMaxOptimizer_l2:
    def __init__(self, e_hat: np.ndarray, pi: np.ndarray, nb: float, n: int, constraint_sum: float, use_path: bool = False):
        """
        ...
        if self.use_path:
            self._path_optimizer = PathCurvatureOptimizer_l2(e_hat, pi, nb, n, constraint_sum)
